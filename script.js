    ...
    list.forEach((item, index) => {
        ...
        const baris = `
            <tr>
                ...
                <td>${item.keterangan}${item.riwayat_bayar ? `<br><small style="color:blue;"><i>(${item.riwayat_bayar})</i></small>` : ''}</td>
                ...
                <td align="center">
                    ...
                    <div style="font-size:10px; font-weight:bold;">Sisa: ${sisa.toLocaleString('id-ID')}</div>
                </td>
                <td align="center"><button onclick="hapusData(${index})" style="background:red; color:white; border:none; padding:4px;">X</button></td>
            </tr>`;
...
function inputBayar(index) {
    ...
    if (nominal) {
        let ketBaru = prompt("Metode & Nama (Contoh: Tunai Marlon / Transfer Vr):", "Tunai");
        let tglBayar = prompt("Tanggal Bayar (Tgl-Bln-Thn):", new Date().toLocaleDateString('id-ID').replace(/\//g, '-'));
        let catatan = `${ketBaru} - ${tglBayar}`;
        list[index].riwayat_bayar = list[index].riwayat_bayar ? `${list[index].riwayat_bayar}, lanjut ${catatan}` : catatan;
...
function imporData() { try { const txt = document.getElementById('importDataText').value; localStorage.setItem('data_keuangan', JSON.stringify(JSON.parse(txt))); alert("Sukses!"); location.reload(); } catch(e) { alert("Error!"); } }
